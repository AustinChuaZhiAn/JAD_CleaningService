...
//			postAddBooking(request, response);
	...
	private void postAddBooking(HttpServletRequest request, HttpServletResponse response)
		...
		//int bookingToService_id = btsList.addBookingServiceEntry(booking_id, List<String> servicesInCart);
...
//		int address_id = Integer.parseInt(address_idStr);
		...
		String service_id = String.valueOf(serviceList.getServiceIdByDetails(category_id, service_type_id, frequency_id));
...
//		int booking_id = bookingList.addBooking(user_id, address_id, time, date, cleaner_id, service_id, special_request);
		...
		// get all service type name, frequency option name, category name, and cleaner name
